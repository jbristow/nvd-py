...
def define_subcommand(subparsers):
    kev_parser = subparsers.add_parser("kev", help="Report count of events with a known vulnerability")
    kev_parser.add_argument("--database", default="cve.duck")
    kev_parser.add_argument( "-n", "--not", action="store_true" , help="return count without a known vulnerability", dest="negate")
